...
def check_org_list_response(response,
    ...
    if status_code == 200:
        ...
        printerr_exit('401 status from server - ' +
                      'probably caused by using an invalid PAT')
    else:
...
def click_complete_setup(driver, debug):
    ...
    if debug:
        printdebug(
            "finding the 'Complete setup' button and " +
...
def click_next_under_org_name_field(driver, debug):
    ...
    if debug:
        printdebug(
            "finding the Next button and waiting for it to be clickable")
...
def create_org(config, debug):
    """create an organization"""
    driver = setup_webdriver(config, debug)
    login(config, driver, debug)
    make_sure_org_does_not_already_exist(config, debug)
...
def make_sure_org_does_not_already_exist(config, debug):
