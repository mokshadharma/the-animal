...
from .common import printdebug, printerr_exit, validate_url
...
def validate_classic_pat_config_section(config, config_file, debug):
    ...
    if 'classic-pat' in config:
        validate_note_in_classic_pat_section(
            config,
            config_file,
            debug)
        validate_scopes_in_classic_pat_section(
            config,
            config_file,
            debug)
        validate_repo_in_classic_pat_scopes_section(
            config,
            config_file,
            debug)
        validate_workflow_in_classic_pat_scopes_section(
            config,
            config_file,
            debug)
        validate_write_packages_scope_for_classic_pat(
            config,
            config_file,
            debug)
        validate_read_packages_scope_for_classic_pat(
            config,
            config_file,
            debug)
        validate_repo_scope_in_classic_pat_scopes_section(
            config,
            config_file,
            debug)
        validate_repo_status_in_classic_pat_scopes_repo_scopes_section(
            config,
            config_file,
            debug)
        validate_repo_deployment_in_classic_pat_scopes_repo_scopes(
            config,
            config_file,
            debug)
        validate_public_repo_in_classic_pat_scopes_repo_scopes(
            config,
            config_file,
            debug)
        validate_repo_invite_in_classic_pat_scopes_repo_scopes(
            config,
            config_file,
            debug)
        validate_security_events_in_classic_pat_scopes_repo_scopes(
            config,
            config_file,
            debug)
    else:
...
def validate_ghes_config_section(config, config_file, debug):
    ...
    if 'server' in config:
        if 'password' not in config['server']:
            printerr_exit(
                'password must exist in the [server] ' +
                f"section in {config_file}")
        if 'url' not in config['server']:
            printerr_exit(
                f"url must exist in the [server] section in {config_file}")
        if 'username' not in config['server']:
            printerr_exit(
                'username must exist in the [server] section ' +
                f"in {config_file}")
    else:
        printerr_exit(f"[server] section must exist in {config_file}")
...
def validate_public_repo_in_classic_pat_scopes_repo_scopes(
        config,
        config_file,
        debug):
    ...
    if 'public_repo' not in config['classic-pat']['scopes']['repo-scope']:
        section = '[classic-pat.scopes.repo-scope]'
        msg = f"public_repo must exist in the '{section}' section " + \
            f"in {config_file}"
        printerr_exit(msg)
    else:
        if not isinstance(
...
def validate_repo_deployment_in_classic_pat_scopes_repo_scopes(config,
    ...
    if 'repo_deployment' not in config['classic-pat']['scopes']['repo-scope']:
        section = '[classic-pat.scopes.repo-scope]'
        msg = f"repo_deployment must exist in the '{section}' section " + \
            f"in {config_file}"
        printerr_exit(msg)
    else:
        scopes = config['classic-pat']['scopes']
        if not isinstance(
                scopes['repo-scope']['repo_deployment'],
                bool):
            section = '[classic-pat.scopes.repo-scope]'
            msg = f"repo_deployment in the '{section}' section " + \
                f"must be true or false in {config_file}"
            printerr_exit(msg)
...
def validate_repo_invite_in_classic_pat_scopes_repo_scopes(
        config,
        config_file,
        debug):
    ...
    if 'repo:invite' not in config['classic-pat']['scopes']['repo-scope']:
        section = '[classic-pat.scopes.repo-scope]'
        msg = f"repo:invite must exist in the '{section}' section " + \
            f"in {config_file}"
        printerr_exit(msg)
    else:
        if not isinstance(
...
def validate_repo_scope_in_classic_pat_scopes_section(
        config,
        config_file,
        debug):
...
def validate_repo_status_in_classic_pat_scopes_repo_scopes_section(config,
    ...
    if 'repo:status' not in config['classic-pat']['scopes']['repo-scope']:
        section = '[classic-pat.scopes.repo-scope]'
        msg = f"repo:status must exist in the '{section}' section " + \
            f"in {config_file}"
        printerr_exit(msg)
    else:
        if not isinstance(
...
def validate_security_events_in_classic_pat_scopes_repo_scopes(config,
    ...
    if 'security_events' not in config['classic-pat']['scopes']['repo-scope']:
        section = '[classic-pat.scopes.repo-scope]'
        msg = f"security_events must exist in the '{section}' section " + \
            f"in {config_file}"
        printerr_exit(msg)
    else:
        scopes = config['classic-pat']['scopes']
        if not isinstance(scopes['repo-scope']['security_events'], bool):
            section = '[classic-pat.scopes.repo-scope]'
            msg = f"security_events in the '{section}' section " + \
                f"must be true or false in {config_file}"
            printerr_exit(msg)
...
def validate_selenium_config_section(config, config_file, debug):
    ...
    if debug:
        msg = f"validating selenium section in config file '{config_file}'"
        printdebug(msg)
...
def validate_login_page(driver, debug):
    ...
    try:
        ...
        printerr_exit('Unexpected web page')
    else:
...
def validate_read_packages_scope_for_classic_pat(config, config_file, debug):
    ...
    if 'read:packages' not in config['classic-pat']['scopes']:
        ...
        if not isinstance(
                config['classic-pat']['scopes']['read:packages'],
                bool):
            printerr_exit(
                'read:packages in the [classic-pat.scopes] section ' +
                f"must be true or false in {config_file}")
...
def validate_write_packages_scope_for_classic_pat(config, config_file, debug):
    ...
    if debug:
        msg = "validating write:packages scope for ['classic-pat.scopes']"
        printdebug(msg)
    if 'write:packages' not in config['classic-pat']['scopes']:
        ...
        if not isinstance(
                config['classic-pat']['scopes']['write:packages'],
                bool):
            printerr_exit(
                'write:packages in the [classic-pat.scopes] section ' +
                f"must be true or false in {config_file}")


def validate_workflow_in_classic_pat_scopes_section(config,
                                                    config_file,
                                                    debug):
